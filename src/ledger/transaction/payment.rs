...
impl<'a> Payment<'a> {
    ...
    fn record_transaction_date_entries_and_return_deferred(
        &self,
        payee_account_code: &AccrualAccount,
        gl: &mut GeneralLedger,
    ) -> (USD, Vec<(DateTime<Utc>, USD)>) {
        if self.effective_on >= self.payee_service_start_date.unwrap() {
            let days_into_service_period = self.effective_on
                .signed_duration_since(self.payee_service_start_date.unwrap());
            let mut days_exclusive: usize =
                (days_into_service_period.to_std().unwrap().as_secs() / 86_400) as usize;
            ...
            let creditable_ar = ar_days
                .iter()
                .fold(USD::zero(), |sum, date_amount| sum + date_amount.1);
...
impl<'a> Transaction<'a> for Payment<'a> {
    ...
    fn process_cash(&self, payee_account_code: &CashAccount, gl: &mut GeneralLedger) {
        gl.record_double_entry(
            ...
            self.amount,
            &self.account_code,
            &payee_account_code.revenue_code,
        );
    }
    ...
    fn process_accrual(&self, payee_account_code: &AccrualAccount, gl: &mut GeneralLedger) {
        if self.effective_on >= self.payee_effective_on {
            ...
            gl.record_double_entry(
                self.payee_effective_on.naive_utc().date(),
                self.amount,
                &payee_account_code.deferred_code,
                &payee_account_code.accounts_receivable_code,
            );
    ...
    fn process_daily_accrual(&self, payee_account_code: &AccrualAccount, gl: &mut GeneralLedger) {
        ...
        let (deferred_amount, leftover_days) =
            self.record_transaction_date_entries_and_return_deferred(payee_account_code, gl);
