...
impl Payment {
    ...
    fn record_transaction_date_entries_and_return_deferred(&self, gl: &mut GeneralLedger) -> (USD, Vec<(DateTime<Utc>, USD)>) {
        if self.effective_on >= self.payee_service_start_date.unwrap() {
            ...
            // Prepay
            gl.record_double_entry(self.effective_on.naive_utc().date(), self.amount, &self.account_code, &account_map::deferred_code(&self.payee_account_code));
